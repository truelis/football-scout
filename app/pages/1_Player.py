...
players = q("SELECT player_id, player_name, current_club_name, league_name "
...
fig = go.Figure(go.Bar(
    x=[row["performance_score"], row["trajectory_score"], row["availability_score"]],
    y=["Performance", "Trajectory", "Availability"],
    orientation="h", text=[f"{row[k]:.0f}" for k in
                           ["performance_score", "trajectory_score", "availability_score"]],
))
fig.update_layout(height=220, xaxis_range=[0, 100], margin=dict(l=0, r=0, t=10, b=10))
st.plotly_chart(fig, use_container_width=True)

# ---- market value history ----
st.subheader("Market value history")
hist = q("SELECT valuation_date, market_value_eur FROM fct_player_valuations "
         "WHERE player_id = ? ORDER BY valuation_date", [pid])
if len(hist) > 1:
...
st.dataframe(
    q("""SELECT season, league_name, appearances, minutes_played, goals, assists,
                round(ga_per90, 2) AS ga_per90
         FROM fct_player_season WHERE player_id = ? ORDER BY season DESC""", [pid]),
